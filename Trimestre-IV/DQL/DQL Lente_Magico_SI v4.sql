...
SELECT d.primer_nombre, d.primer_apellido, c.telefono
FROM cliente c
JOIN datos_personales d ON c.id_datos_cliente = d.id
...
SELECT p.codigo_producto, p.nombre, i.stock_actual, i.stock_minimo
FROM producto p
...
SELECT pr.razon_social, pr.tipo, c.id_compra, c.num_comprobante, c.fecha_compra
FROM compra c
RIGHT JOIN proveedor pr ON c.id_proveedor = pr.id_proveedor;
...
SELECT nombre, codigo_producto, precio_venta
FROM producto
WHERE precio_venta > (
